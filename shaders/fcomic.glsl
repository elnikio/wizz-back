...
// 1. DIFFUSE SHADOW, FOLLOWING A FIXED PATTERN, DEFINED BY THE STYLE OF THE COMIC, IRRESPECTIVE OF THE MATERIAL.
// 2. AMBIENT OCCLUSION SHADOW - A SHARP GRADIENT OF THE HEIGHTMAP, OR AO MAP OF THE PARTICULAR TEXTURE.
// EACH MATERIAL IS COMPRISED OF:
// 1. COLOR MAP, DEFINING THE UNLIT COLORS OF THE SURFACE
// 2. AMBIENT OR HEIGHT MAP, USED FOR AMBIENT OCCLUSION SHADOW
// STYLE SHADOW:
// 1. SELECT A SMOOTH, TILING HEIGHTMAP FOR THE BRUSH
// 2. HEIGHTSCAN IT BASED ON DARKNESS OF THE SHADOW
// BEFORE I DO THIS, I SHOULD IMPLEMENT SMOOTH PHONG SHADOWS IN THE GENERIC OBJECT RENDERER
...
void main (void) {
	...
	if (length(varyingColor) < 1.05) {
		if (int(gl_FragCoord.y) % 4 > 1)
			color = vec4 (0.0, 0.0, 0.0, 1.0);
	}
	else if (length(cross(normalize(eye_interpolated), normalize(normal_interpolated))) > 0.95) {
		color = color + vec4 (0.25, 0.25, 0.25, 0.0);
		if (length(cross(normalize(eye_interpolated), normalize(normal_interpolated))) > 0.99)
			color = color + vec4 (0.55, 0.55, 0.55, 0.0);
	}
}
